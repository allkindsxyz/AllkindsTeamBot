...
async def start_communicator_bot() -> None:
    ...
    try:
        ...
        try:
            ...
            env_vars = ", ".join([f"{k}={v[:3]}..." for k, v in os.environ.items() \
                                if k.endswith("TOKEN") or k.endswith("USERNAME")])
        ...
        max_retries = 10
        base_delay = 2
        max_polling_retries = 3  # Number of retries for each polling attempt

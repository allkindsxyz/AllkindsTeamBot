...
from src.db.utils.session_management import with_transaction_lock, release_transaction_lock, ensure_active_session
from src.db.utils.state_persistence import save_critical_state, load_critical_state, delete_critical_state
...
from src.db.repositories.chat_repo import get_chat_by_participants
from src.core.diagnostics import get_diagnostics_report, IS_RAILWAY
from src.db.utils.session_management import with_transaction_lock, release_transaction_lock
...
async def on_add_question(message: types.Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    prompt_msg = await message.answer("Please enter your yes/no question.\n\n"
...
async def on_go_to_group(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    logger.info(f"DEBUG: on_go_to_group called, session type: {type(session)}")
    """Handle go to group button click."""
    logger.info(f"[DEBUG_TEAM] Entering on_go_to_group for user {callback.from_user.id}, data={callback.data}")
    logger.info(f"[DEBUG] Entering on_go_to_group handler for user {callback.from_user.id}")
    logger.info(f"[DEBUG] Callback data: '{callback.data}'")
    logger.info(f"[DEBUG] Session available: {session is not None}, type: {type(session) if session else 'None'}")
    ...
    try:
        ...
        try:
            ...
            parts = callback_data.split(":")
            if len(parts) < 2:
                logger.error(f"[ERROR] Invalid callback data format: {callback_data}")
                await callback.message.answer("Error: Invalid callback data format. Please try again.")
                return
                
            group_id = int(parts[1])
            logger.info(f"[DEBUG] Extracted group_id={group_id} from callback data")
        except (ValueError, IndexError) as e:
            logger.error(f"[ERROR] Error parsing callback data '{callback.data}': {e}")
        ...
        try:
            group = await group_repo.get(session, group_id)
            if not group:
                logger.error(f"[ERROR] Group with ID {group_id} not found in database")
                await callback.message.answer("Error: Team not found. Please try again.")
                return
            
            logger.info(f"[DEBUG] Found group: id={group.id}, name={group.name}, creator_id={group.creator_id}")
        except Exception as e:
            logger.exception(f"[ERROR] Error retrieving group with ID {group_id}: {e}")
        ...
        logger.info(f"[DEBUG] Found user: id={db_user.id}, telegram_id={user_tg.id}")
            
        # Check if user is a member of this group
        is_member = await group_repo.is_user_in_group(session, db_user.id, group_id)
        if not is_member:
            logger.warning(f"[WARNING] User {db_user.id} is not a member of group {group_id}, adding them...")
            try:
                # Determine if they should be a creator or regular member
                role = MemberRole.MEMBER
                if group.creator_id == db_user.id:
                    role = MemberRole.CREATOR
                    logger.info(f"[DEBUG] User {db_user.id} is the creator of group {group_id}, assigning CREATOR role")
                
                # Add them to the group
                await group_repo.add_user_to_group(session, db_user.id, group_id, role=role)
                logger.info(f"[DEBUG] Added user {db_user.id} to group {group_id} with role {role}")
                await session.commit()
            except Exception as e:
                logger.exception(f"[ERROR] Error adding user {db_user.id} to group {group_id}: {e}")
                await callback.message.answer("Error joining the team. Please try again.")
                return
        ...
        # Log the action
        logger.info(f"[DEBUG] User {callback.from_user.id} (DB ID: {db_user.id}) clicked Go to group button for group {group_id} ({group.name})")
        ...
        try:
            ...
            logger.error(f"[ERROR] Error editing message: {e}")
            # If editing fails, send a new message
        ...
        try:
            logger.info(f"[DEBUG] Showing group menu for user {callback.from_user.id} in group {group_id}")
            await show_group_menu(callback.message, group_id, group.name, state, session=session)
        except Exception as e:
            logger.exception(f"[ERROR] Error showing group menu: {e}")
            await callback.message.answer("Error showing group menu. Please try again.")
            return
        
        # After showing the menu, trigger the questions view
        try:
            logger.info(f"[DEBUG] Showing questions for user {callback.from_user.id} in group {group_id}")
            await on_show_questions(callback.message, state, session)
        except Exception as e:
            logger.exception(f"[ERROR] Error showing questions: {e}")
            await callback.message.answer("Error loading questions. Please try again.")
    except Exception as e:
        logger.exception(f"[ERROR] Unhandled error in on_go_to_group: {e}")
        try:

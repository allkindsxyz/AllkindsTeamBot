...
def get_api_credentials(service: str) -> Dict[str, str]:
    ...
    Args:
        service: Service name (e.g., "openai", "telegram", "pinecone", "communicator")
    ...
    elif service.lower() == "communicator":
        # Try to get the token, but don't exit if not found
        token = get_optional_env("COMMUNICATOR_BOT_TOKEN")
        if token:
            credentials["bot_token"] = token
        
        # Get the username with a fallback
        username = get_optional_env("COMMUNICATOR_BOT_USERNAME", "AllkindsChat")

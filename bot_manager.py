...
COMMUNICATOR_BOT_MODULE = "src.communicator_bot.main"
MAIN_PID_FILE = "main_bot.pid"
COMM_PID_FILE = "communicator_bot.pid"
LOGS_DIR = "logs"
    ...
    for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            ...
            if (MAIN_BOT_MODULE in cmdline or 
                COMMUNICATOR_BOT_MODULE in cmdline):
                proc.terminate()
...
async def reset_webhooks():
    ...
    # Comment out communicator bot reset to make the main bot start independently
    """
    # Reset communicator bot webhook
    print("Resetting communicator bot webhook...")
    from reset_communicator_bot import verify_token, reset_bot
    token_valid = await verify_token()
    if token_valid:
        success = await reset_bot()
        if success:
            print("Communicator bot reset successfully")
        else:
            print("Failed to reset communicator bot")
    else:
        comm_result = await delete_webhook(COMMUNICATOR_BOT_TOKEN)
        if comm_result:
            print(f"Communicator bot webhook reset: {json.dumps(comm_result)}")
    """
...
def check_bots_status():
    ...
    # Check communicator bot
    comm_running, comm_pid, comm_status = get_process_status(COMM_PID_FILE)
    if comm_running:
        print(f"✅ Communicator bot is running (PID: {comm_pid}, Status: {comm_status})")
    else:
        print(f"❌ Communicator bot is not running ({comm_status})")
    ...
    print("Run 'python bot_manager.py logs communicator' to view communicator bot logs")
    
    return main_running, comm_running

def view_logs(bot_type, lines=50):
    """View bot logs"""
    log_file = f"{LOGS_DIR}/main_bot.log" if bot_type == 'main' else f"{LOGS_DIR}/communicator_bot.log"
...
async def start_bots(which_bots='both'):
    ...
    # Comment out communicator bot start to ensure main bot works independently
    """
    if which_bots in ['communicator', 'both']:
        start_bot(
            COMMUNICATOR_BOT_MODULE,
            COMM_PID_FILE,
            f"{LOGS_DIR}/communicator_bot.log"
        )
    """
...
def stop_bots():
    ...
    if comm_killed:
        print("Communicator bot stopped.")
    else:
        print("Communicator bot was not running or could not be stopped.")
...
def parse_args():
    ...
    start_parser.add_argument('bot', nargs='?', choices=['main', 'communicator', 'both'], 
    ...
    logs_parser.add_argument('bot', choices=['main', 'communicator'], 

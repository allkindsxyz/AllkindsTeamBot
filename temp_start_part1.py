...
async def on_confirm_delete_question(callback: types.CallbackQuery, state: FSMContext, session: AsyncSession) -> None:
    ...
    # Update the message
    await callback.message.edit_text("Question deleted.")
    ...
    try:
        ...
        try:
            ...
            if callback.message and callback.message.text and callback.message.text.startswith("<b>📝 New Question in"):
                # Keep the question but update it to show your answer
                await callback.message.edit_text(
                    text=question.text,
                    reply_markup=single_button_keyboard
                )
                logger.info(f"Updated notification message for question {question_id} to show answer")
            else:
...
async def handle_start_anon_chat(query: types.CallbackQuery, state: FSMContext, bot: Bot) -> None:
    ...
    # Temporarily disabled communicator bot integration
    """
    payload = f"match_{user_id}_{matched_user_id}"
    try:
        # Get settings inside the function where needed
        communicator_bot_username = settings.COMMUNICATOR_BOT_USERNAME
        if not communicator_bot_username:
             raise ValueError("COMMUNICATOR_BOT_USERNAME is not configured in settings.")

        start_link = f"https://t.me/{communicator_bot_username}?start={payload}"
        chat_button = types.InlineKeyboardButton(text="➡️ Start Chatting", url=start_link)
        keyboard = types.InlineKeyboardMarkup(inline_keyboard=[[chat_button]])

        await query.message.edit_text(
            "Great! Click the button below to join the anonymous chat.",
            reply_markup=keyboard
        )
        ...
        try:
            await bot.send_message(
                ...
                text=f"🎉 You've been matched with a user! Cohesion score: **{score:.2f}**\n\n" \
                     f"They have started an anonymous chat. Click below to join:",
                reply_markup=keyboard
            )
            logger.info(f"Sent chat invitation to matched user {matched_user_id}")
        except Exception as e:
            logger.error(f"Failed to send chat invitation to matched user {matched_user_id}: {e}")
            await query.message.reply("(Note: Could not notify the matched user. They might have blocked the bot.)")

    except Exception as e:
        logger.exception(f"Error creating or sending deep link for match {user_id}-{matched_user_id}: {e}")
        await query.message.edit_text("Sorry, an error occurred while trying to initiate the chat. Please try again.")
    """
    
    # Simple temporary message
    await query.message.edit_text(
        f"🎉 You've been matched with a user! Cohesion score: **{score:.2f}**\n\n" +
        "Anonymous chat functionality is temporarily disabled. This feature will be available soon!"
    )
...
async def handle_cancel_match(query: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await query.message.edit_text("Match cancelled.")
    await state.clear()
...
async def on_find_match(message: types.Message, state: FSMContext, session: AsyncSession) -> None:
    ...
    await show_group_menu(message, group_id, group_name, state, current_section="matches")
    ...
    await message.answer("Finding matches for you... Please wait.")
    
    # Set state to waiting for match results
    ...
    matched_user_id, score, common_questions = match_result
    ...
    match_text = (
        f"🎉 Found a match! Cohesion score: **{score:.2f}**\n\n"
        f"You and this person agree on {len(common_questions)} questions."
    )
    ...
    # Set state to waiting for chat confirmation
    await state.set_state(MatchingStates.waiting_for_chat_confirmation)
...
def register_handlers(dp: Dispatcher) -> None:
    ...
    dp.callback_query.register(
        ...
        MatchingStates.waiting_for_chat_confirmation
    )
    dp.callback_query.register(
        handle_cancel_match,
        F.data == "cancel_match",
    ...
    dp.callback_query.register(on_edit_add_question, F.data == "edit_add_question")
